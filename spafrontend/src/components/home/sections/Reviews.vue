...
toRef(() => props.subtitle);

const defaultColors = ['fill-gray-300','fill-gray-500'];
const colorColors = ['fill-yellow-300','fill-yellow-600'];


const avgRating = ref(0);
const actualAvgRating = ref(0);
const lenReview = ref(0);
const reviewUploads = ref([]);
...
watch(() => props.collection, (c, b) => { 

    lenReview.value = c.length;
    let sumRating = 0;
    if (lenReview.value==0) { 
        props.subtitle = 'No review yet.'
    } else {
        ...
        actualAvgRating.value = sumRating/lenReview.value;
        colorValues.value.length =0;
        for (let x=0; x < 5 ; x++){
            if ((x+1)<=avgRating.value) {
...
const reformatUploads = (review_id, photos) => {
    ...
    // console.log("uploads: ", reviewUploads.value, "Photos: ", photos);
}
...
<template>
    <div class="mb-4 w-full flex">
        <div class="flex justify-start w-[50%]">
            <h1 class="font-semibold text-2xl my-2 flex items-center">
                <span class="bg-cyan-300 rounded-lg py-2 px-3 text-sm mr-2">{{ lenReview }}</span>
                {{ title }}
            </h1>
            <p class="">{{ subtitle }}</p>
        </div>
        <div class="flex justify-end w-[50%] items-center">
            <ul class="flex items-center justify-center gap-1 pl-2">
                ...
                <li><span class="pl-2 font-semibold text-lg">{{ actualAvgRating }}/5</span></li>
            </ul>
            
        </div>
    </div>
    <div class="mb-4 grid grid-cols-2 gap-4" v-if="collection.length>0">
        <div v-for="item in collection">
            <div class="rounded-lg bg-gray-200 p-6 relative">
                <div class="w-full mb-4">
                    <p class="font-semibold">By: {{ item.user.name }}</p>
                    <p>{{ toReadableDate(item.user.created_at) }}</p>
                </div>
                <p class="mb-4">{{ item.review }}</p>
                <div class="absolute top-5 right-4 p-2 bg-yellow-300/90 font-semibold text-sm items-center flex rounded-lg">
                    <span class="mr-2">
                ...
                <div class="mb-4 grid grid-cols-3 gap-1 w-full">                    
                    <template v-for="img in reviewUploads">
                        <div class="flex max-h-22" v-if="img.id == item.id">
